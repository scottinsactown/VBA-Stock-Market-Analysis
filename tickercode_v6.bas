...
'****not using percent_change as of 9/27 pm:
'Dim percent_change As Double

Dim lastrow As Double
lastrow = Cells(Rows.Count, 1).End(xlUp).row

'this variable is for the locaiton in the summary table
Dim row_summary As Double
row_summary = 2

'this variable is for the current row
Dim row_search As Double
For row_search = 2 To lastrow + 1

'populate summary stats

    'current ticker ID volume counted
...
For row_search = 2 To lastrow
    ...
    'green positive change (does not include zero)
...
'Open new worksheet and repeat
Next ws

'This returns you to the starting sheet
